...
public class WordTable {
    public static final String WORD_TABLE_NAME = "tblWord";
    
    public static void createWordTable(Connection conn, boolean bClean) throws IOException {
        try {
            Statement stmt = conn.createStatement();
            if (bClean) {
                stmt.execute("drop table if exists " + WORD_TABLE_NAME);
            }
            stmt.execute("create table if not exists " + WORD_TABLE_NAME
    ...
    public static void insertWord(Connection conn, String word) throws IOException {
        try {
            PreparedStatement pstmt = conn.prepareStatement("insert into " + WORD_TABLE_NAME 
    ...
    public static int getRegisteredWordCount(Connection conn) throws IOException {
        try {
            PreparedStatement pstmt = conn.prepareStatement("select count(*) as cnt from " + WORD_TABLE_NAME);
    ...
    public static Hashtable getHashTable(Connection conn) throws IOException {
        ...
        try {
            PreparedStatement pstmt = conn.prepareStatement("select * from " + WORD_TABLE_NAME);
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
    ...
    public static void deleteWord(Connection conn, String word) throws IOException {
        try {
            PreparedStatement pstmt = conn.prepareStatement("delete from " + WORD_TABLE_NAME 
                    + " where word = ?");
            pstmt.setString(1, word);
            pstmt.executeUpdate();
